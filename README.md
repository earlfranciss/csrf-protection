...
## Deploying this so you can demo it live

The cleanest way to actually demonstrate cross-site behavior in
production (rather than the `localhost` vs `127.0.0.1` trick) is to
deploy the app and the attacker page to **two different real domains**.

1. Push this repo to GitHub (see below).
2. Deploy `server/` to a free host that supports Node — [Render](https://render.com),
   [Railway](https://railway.app), or [Fly.io](https://fly.io) all work well.
   - Set the `MODE` environment variable to `vulnerable` or `protected`
     in the host's dashboard to switch versions.
   - Note: real deployments get HTTPS automatically, which is worth
     knowing about — cookies marked `Secure` only ever travel over HTTPS,
     and `SameSite=None` (intentionally *disabling* same-site protection,
     sometimes needed for legitimate embedded widgets) requires `Secure`
     to work in modern browsers at all.
3. Deploy `attacker-site/attack.html` as a static site (Netlify, Vercel,
   or GitHub Pages all work) on a different domain, and update the
   `action`/`src` URLs inside `attack.html` to point at your deployed
   app's real URL instead of `localhost:3000`.

---

## Documenting this on GitHub

Suggested repo structure once you push it:

```bash
cd csrf-demo
git init
git add .
git commit -m "Initial commit: CSRF vulnerable/protected demo"
git branch -M main
git remote add origin https://github.com/<your-username>/csrf-demo.git
git push -u origin main
```

For the repo itself, a few things worth adding once it's up:

- **This README as-is** works as the main write-up — it already covers
  what CSRF is, how the attack works, and what each fix does.
- **A short GIF or screen recording** of the attack succeeding in
  vulnerable mode and failing in protected mode is the single most
  convincing thing you can add to a security-focused portfolio project —
  screenshots of a 403 response are good, but watching the email
  actually change (and then not change) is what sticks with a reviewer.
- **A "Learnings" section** in your own words — what surprised you, what
  the browser's default `SameSite=Lax` behavior means for real-world risk
  today, what you'd add if this were a real production app (e.g. also
  requiring re-authentication for sensitive changes like email/password,
  rate limiting the endpoint, logging changes for audit).
